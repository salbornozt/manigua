    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <form action="crud/update_article.php" method="post" enctype="multipart/form-data">
          <div class="card-header py-3">
              <div class="newspaper">
              <br> 
              <h6 class="m-0 font-weight-bold text-success" align="center">Articulos Científicos</h6>
            ...
            <div class="card-body">
                    ...
                    <?php
                      $sql = "SELECT a.ssn, a.title, a.publication_date, a.dir, doc.pages, doc.editorial_code, ed.editorial_name
                      from documents doc, article a, editorial ed
                      where 
                      doc.cod_doc = a.cod_doc and
                      doc.editorial_code = ed.editorial_code and 
                      doc.cod_doc = $cod_doc;";
                      $result = pg_query($conexion, $sql);
                      while ($mostrar=pg_fetch_row($result))
                      {
                        ...
                        $doc_dir = $mostrar[3];
                        $doc_pages = $mostrar[4];
                        $doc_editorial_code = $mostrar[5];
                        $doc_editorial_name = $mostrar[6];                        
                      
                      }
                      echo "<input type='text' id='cod_doc' name='cod_doc'  style = 'display:none' value='".$cod_doc."'>";
                    ?>

	              <div class="form-group"> 
	              	<label for="full_name_id" class="font-weight-bold" >Titulo</label>
	              	<?php echo "<input type='text' class='form-control' id='doc_title' name='doc_title' value='".$doc_title."' required>" ?>
	              </div>	

	              <div class="form-group"> <!-- Street 1 -->
	              	<label for="street1_id" class="font-weight-bold" >SSN</label>
	              	<?php echo "<input type='text' class='form-control' id='doc_isbn' name='doc_isbn' value='".$doc_isbn."' readonly>" ?>
	              </div>					
                ...
                <div class="newspaper">
                  <div class="form-group"> <!-- Street 2 -->
	                	...
	                	<?php echo "<input type='date' class='form-control' id='doc_publication_date' name='doc_publication_date' value='".$doc_date."' required>" ?>
                  </div> 
                  <div class="form-group"> <!-- Street 2 -->
	                	<label for="street2_id" class="font-weight-bold" >Número de Páginas</label>
	                	<?php echo "<input type='nuumber' class='form-control' id='doc_pages' name='doc_pages' value='".$doc_pages."' onkeypress='return event.charCode >= 48 && event.charCode <= 57' required>" ?>
                  </div>
                  <div class="form-group"> <!-- Street 2 -->
	              	  <label for="street2_id" class="font-weight-bold">Archivo</label>
	              	  <?php echo "<input type='file' class='form-control' id='doc_file' name='doc_file' accept='.pdf'>" ?>
                </div>
                </div>
                ...
                <div id="author_space" class="form-group">
                        <?php
                          $sql = "SELECT a.author_id, a.author_name
                          from documents d, author a, document_author da
                          where 
                          d.cod_doc = da.cod_doc and
                          da.author_id = a.author_id and
                          d.cod_doc = $cod_doc;";
                          $result = pg_query($conexion, $sql);
                          $rows = pg_num_rows($result);
                          $aux = 1;
                          echo "<input type='text' id='author_num' name='author_num'  style = 'display:none' value='".$rows."'>";
                          while ($mostrar=pg_fetch_row($result))
                          {
                            echo "<div id = 'author_form' class='form-group'>";    
                            echo "<div class='form-group'>";
                            echo "<label for='street1_id' class='font-weight-bold'>Autor ".$aux."</label>";
                            echo "<select  class='form-control' name='selected_author".$aux."' >";
                            ...
                            {
                              if($mostrar[0] == $mostrar2[0]){
                                echo "<option selected value='".$mostrar2[0]."'>".$mostrar2[1]."</option>" ;

                              }else{
                                echo "<option value='".$mostrar2[0]."'>".$mostrar2[1]."</option>" ;
                ...
                <div class="form-group">
                  ...
                  <div class="form-group"> 
                        ...
                        <?php
                          ...
                          while ($mostrar=pg_fetch_row($result))
                          {
                            if($doc_editorial_code == $mostrar[0]){
                              echo "<option selected value=".$mostrar[0].">".$mostrar[1]."</option>";
                            }else{
