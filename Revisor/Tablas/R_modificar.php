...
$sql="SELECT user_name, pass FROM customer WHERE user_code ='".$cod."';";
...
if (isset($_POST['submit'])) 
{
  ...
  if ($pcAN == $pAA) 
  {
      ...
      if (preg_match($patron, $pN))
      {
        if ($pcN == $pcCN) 
        {
          $sql1="SELECT user_name, email FROM customer WHERE user_code='".$cod."';";
          ...
          while ($mostrar=pg_fetch_row($r1))
          {
            ...
            try 
            {
              ...
              if($mail->send())
              {
                $sql="UPDATE customer SET pass='$pcCN' WHERE user_code='".$cod."';";
